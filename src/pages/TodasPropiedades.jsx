      ...
      <div className="card h-100 shadow-sm border-0 rounded-3 overflow-hidden hover-shadow">
          ...
          {imagenes.length > 1 && (
            <>
              <button
                ...
                onClick={handlePrev}
                style={{ backgroundColor: "rgba(0,0,0,0.5)", border: "none", color: "#fff", width: "32px", height: "32px", borderRadius: "50%", opacity: 0.8, cursor: "pointer", zIndex: 10 }}
        ...
        <div className="card-body d-flex flex-column">
          ...
          <div className="d-flex flex-wrap gap-2 text-muted small mb-1">
            {propiedad.habitaciones && <span className="d-inline-flex align-items-center"><FaBed className="me-1" /> {propiedad.habitaciones} Hab.</span>}
            {propiedad.baños && <span className="d-inline-flex align-items-center"><FaBath className="me-1" /> {propiedad.baños} Baño{propiedad.baños > 1 ? "s" : ""}</span>}
            {propiedad.m2 && <span className="d-inline-flex align-items-center"><FaRulerCombined className="me-1" /> {propiedad.m2} m²</span>}
            {propiedad.cochera && <span className="d-inline-flex align-items-center"><FaCar className="me-1" /> {propiedad.cochera} Coch.</span>}
          </div>

          <div className="d-flex flex-wrap gap-2 mb-3">
            {propiedad.internet && <span className="badge rounded-pill bg-light text-dark border"><FaWifi className="me-1" /> Internet</span>}
            {propiedad.pileta && <span className="badge rounded-pill bg-light text-dark border"><FaSwimmingPool className="me-1" /> Pileta</span>}
            {propiedad.gasNatural && <span className="badge rounded-pill bg-light text-dark border"><FaBurn className="me-1" /> Gas natural</span>}
            {propiedad.patio && <span className="badge rounded-pill bg-light text-dark border"><FaLeaf className="me-1" /> Patio</span>}
          </div>

          {propiedad.direccion && (
            <div className="mb-3 mt-auto">
              <div className="text-muted small d-flex align-items-start" style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", textOverflow: "ellipsis", minHeight: "3em" }}>
              ...
              <div className="d-flex align-items-center justify-content-between mt-1" style={{ overflow: "hidden" }}>
                <div className="text-muted small text-truncate" title={`${propiedad.direccion.localidad}, ${propiedad.direccion.provincia}, ${propiedad.direccion.pais}`} style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", marginRight: "8px" }}>
                  {propiedad.direccion.localidad}, {propiedad.direccion.provincia}, {propiedad.direccion.pais}
                </div>
                <a
                  className="btn btn-danger btn-sm d-flex align-items-center justify-content-center"
                  href={propiedad.ubicacionGeo?.lat && propiedad.ubicacionGeo?.lng ? `https://www.google.com/maps?q=${propiedad.ubicacionGeo.lat},${propiedad.ubicacionGeo.lng}` : `https://www.google.com/maps?q=${encodeURIComponent(`${propiedad.direccion.calle} ${propiedad.direccion.localidad} ${propiedad.direccion.provincia} ${propiedad.direccion.pais}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  title="Ver en Google Maps"
                  style={{ minWidth: "110px", padding: "4px 8px", gap: "6px", whiteSpace: "nowrap" }}
                >
                  <img alt="Google Maps" src="https://res.cloudinary.com/dcggcw8df/image/upload/v1755458272/r3kx7npz5muhzio5agq8.png" style={{ width: "20px", height: "20px" }} />
...
const Sidebar = ({ propiedades, subfiltro, onSetSubfiltro, searchTerm, setSearchTerm }) => {
  ...
  const toggleSubtipo = (sub) => {
    ...
    if (subfiltro.includes(sub)) onSetSubfiltro(subfiltro.filter((t) => t !== sub));
    else onSetSubfiltro([...subfiltro, sub]);
  };
  ...
  return (
    <div className="p-2 bg-transparent rounded shadow-sm mb-5 py-2 mt-3">
      ...
      <div className="list-group list-group-flush bg-transparent">
        {subtipos.map((sub) => (
          <button key={sub} className={`list-group-item list-group-item-action rounded-2 mb-2 ${Array.isArray(subfiltro) && subfiltro.includes(sub) ? "active fw-bold" : ""}`} onClick={() => toggleSubtipo(sub)}>
            {sub} ({subCounts[sub] || 0})
          </button>
        ))}

        <button className={`list-group-item list-group-item-action rounded-2 ${Array.isArray(subfiltro) && subfiltro.length === 0 ? "active fw-bold" : ""}`} onClick={() => onSetSubfiltro([])}>
          Todos ({totalSubtipos})
        </button>
      </div>
    </div>
    ...
    <main className="alquileres-page">
      <section className="py-5 bg-light">
        <div className="mx-lg-5 mx-3">
          <h2 className="text-center text-danger mb-4 mx-lg-1" style={{ fontFamily: "Prata, serif" }}>Todas las Propiedades</h2>
          ...
          <div className="row g-4">
            <section className="col-lg-10">
              ...
              {loading ? (
                ...
                <div className="row">
                  {propiedadesFiltradas.map((prop) => <PropertyCard key={prop.id} propiedad={prop} />)}
                </div>
              )}
            </section>
...
export default TodasPropiedades;
